...
The generated `sitemap.xml` includes only public landing language pages. `/app/` is disallowed in `robots.txt` and receives its own static `noindex,nofollow` entry file. Confirm Pages is configured to use GitHub Actions, then test `/`, `/en/`, and `/app/?mode=companion` on the deployed subpath. A committed artifact folder is also portable at `/<repository>/dist/` for branch previews; its app entry is `/<repository>/dist/app/?mode=companion`, but the Actions URL without `/dist/` remains the canonical production address.
