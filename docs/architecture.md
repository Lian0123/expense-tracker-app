...
The Vite entry (`src/main.tsx`) mounts `App`. `App` selects the public landing page or `/app`, owns mode, scene and current mascot event, and passes the shared snapshot to `AppShell`. `CompanionView` and `LedgerView` are sibling projections of the same state, so a write in either view is immediately visible in the other.
...
`assetUrl()` is the only runtime public-asset URL builder. `vite.config.ts` injects the current base path for local root hosting and GitHub Pages repository hosting. The service worker caches the app shell only; it never caches private transaction data as a generated file.
